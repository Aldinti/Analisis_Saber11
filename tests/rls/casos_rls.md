...
> Datos **ficticios**; cuentas de prueba creadas en el tenant de ensayo `aldinti.onmicrosoft.com`
> (el Service no admite UPN inexistentes al asignar roles, ADR-0019). Plan §15.3–§15.6.
...
1. **Pre-validación automática (agente, en el modelo abierto en Power BI Desktop):** se evalúa en DAX la **misma
   ...
   Evidencia: `reports/bi/rls_simulacion_dax.csv`, `reports/bi/supresion_dax_fixture.csv`. Esas evidencias se
   generaron con los UPN `example.org` previos a ADR-0019: la expresión evaluada y los valores esperados son
   los mismos, solo cambia la literal comparada con `USERPRINCIPALNAME()`.
...
3. **Despliegue (pendiente, ADR-0015):** repetir RLS-01..05 en Power BI Service asignando al rol `Rol_Rector`
   las cuentas del tenant de ensayo con permiso *Viewer*. El Service **no acepta UPN que no existan en el
   directorio**, por eso las identidades de esta matriz son cuentas reales del tenant (ADR-0019).
...
|---|---|---|---|---|
| RLS-01 | `rector.abc@aldinti.onmicrosoft.com` | Solo ABC; Evaluados 128; Promedio 407,18 | ✅ ABC · 128 · 407,18 | ✅ Aprobado |
| RLS-02 | `rector.rst@aldinti.onmicrosoft.com` | Solo RST; Evaluados 190; Promedio 402,31 | ✅ RST · 190 · 402,31 | ✅ Aprobado |
| RLS-03 | `no.registrado@aldinti.onmicrosoft.com` | Sin datos del colegio; aviso "Usuario sin colegio asignado" | ✅ 0 colegios · aviso visible | ✅ Aprobado |
| RLS-04 | `rector.multi@aldinti.onmicrosoft.com` (ABC y RST) | Unión ABC + RST; Evaluados 318; Promedio 404,27 | ✅ "ABC, RST" · 318 · 404,27 | ✅ Aprobado |
| RLS-05 | `RECTOR.ABC@ALDINTI.ONMICROSOFT.COM` | Igual que RLS-01 | ✅ igual a RLS-01 (comparación sin distinguir mayúsculas) | ✅ Aprobado |
| RLS-06 | `rector.abc@aldinti.onmicrosoft.com`, tarjeta Promedio Distrito | 405,36, igual para todos los casos | ✅ 405,36 en RLS-01..05; 408 filas de benchmark visibles | ✅ Aprobado |
| RLS-07 | `rector.abc@aldinti.onmicrosoft.com`, tabla `seguridad_rectores` | Solo su fila | ✅ 1 fila (RLS-04: 2 filas) | ✅ Aprobado (tabla oculta: verificado en DAX) |
